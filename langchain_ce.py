...
speaker = "Trump"  # Define the speaker here

# Initialize the LLM
llm = ChatOpenAI(model_name='gpt-4-turbo', api_key=api_key, temperature=0.7)

# Define custom prompts for claim extraction
create_draft_answer_prompt = PromptTemplate(
    input_variables=['question'],
    template='Without any modification extract the exact claims from the following text:\n\n{question}\n\nClaims:'
)
...
check_assertions_prompt = PromptTemplate(
    ...
    template='Here is a list of claims:\n{assertions}\nFor each claim, list the claim and determine if it is measurable or not. If it is measurable, explain how it could be measured.'
)
...
def extract_claims(text):
    ...
    # Add the speaker reference to the result
    final_result = f"Measurable claims by {speaker}:\n{result['result']}"
    return final_result
# Use the function
